...
public class fun_sis {
	...
	public boolean val_sis_adi(long nx1) throws Exception {

		String bc_sql = "SELECT  count (1) > 0 as existe " + "FROM tb_sis_adi " + "WHERE id_sis = '" + nx1 + "'";
	...
	public cla_glo_ad cons_sis_adi(Long nx1) throws Exception {

		cla_glo_ad gra_inp = new cla_glo_ad();

		String bc_sql = "select * FROM tb_sis_adi where id_sis = '" + nx1 + "'";
		PreparedStatement gra_bus = pos_cbd_con.prepareStatement(bc_sql);
	...
	public cla_glo_ad sav_sis_adi(cla_glo_ad gra_bus) throws Exception {
		...
		if (!val_sis_adi(gra_bus.getId_sis())) {

			String bc_sql = "INSERT INTO public.tb_sis_adi("
					+ "id_sis, reg_id, reg_data, reg_alt, reg_data_alt, truefalse, " + "tel_2, email_2, login_id) "
					+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
			...
			String bc_sql = "UPDATE public.tb_sis_adi\r\n"

					+ " SET reg_alt=?, reg_data_alt=?, tel_2=?, email_2=? WHERE id_sis = " + gra_bus.getId_sis() + ";";
