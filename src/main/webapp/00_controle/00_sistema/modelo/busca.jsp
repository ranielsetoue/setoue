...
<form method="post" action="<%=request.getContextPath()%>/lt_sis_log/" onsubmit="return validardados()">
	...
	<div class="container mt-4">
		...
		<!-- Formulário de busca -->
		<form action="busca_sis.jsp" method="get" class="mb-3">
			<label for="termo" class="form-label">Digite o nome, CPF/CNPJ
				ou ID:</label> <input type="text" name="termo" id="termo"
				class="form-control" value="${param.termo}"
				placeholder="Ex: Raniel Assis Setoue">
				...
				<tbody>
					<c:forEach var="item" items="${listaResultados}">
						<tr>
							...
							<td>${item.cnpj_cpf}</td>
							<td><fmt:formatDate value="${item.reg_data}"
									pattern="dd/MM/yyyy HH:mm:ss" /></td>
							<td>${item.no_fan}</td>
							<td>${item.end_rua}, ${item.end_num} ${item.end_com},
								${item.end_bar} - ${item.end_mun}/${item.end_uf} CEP:
								${item.end_cep}</td>
							<td>${item.tel_1}</td>
							<td>${item.email_1}</td>
							<td>${item.obs}</td>
